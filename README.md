...
| 役割 | 担当 | 理由 |
| --- | --- | --- |
| 写真 → 8×8盤面の読み取り | マルチモーダルLLM (Gemma 3 27B, Google AI Studio無料枠) | 画像からの構造化データ抽出はLLMの得意分野 |
...
無料リクエスト枠があるモデル(Gemma 3 / Gemini 2.0 Flash など)を利用する
...
API routeには `maxDuration = 30` を設定済みです。Vercel Hobbyプランの
上限に合わせて必要であれば調整してください。
